...
char* tree_to_2a_code(TreeNode* tn, struct symbolTableTreeNode* symtable, struct symbolTableTreeNode* symtable_root, char* code_2a, struct pile* stack_tmp){
	if(tree_length(tn->left) > tree_length(tn->right)){
		if(tn->left != NULL){
			addr_left = tree_to_2a_code(tn->left, symtable, symtable_root, code_2a, stack_tmp);
		}

		if(tn->right != NULL){
			addr_right = tree_to_2a_code(tn->right, symtable, symtable_root, code_2a, stack_tmp);
		}
	}
